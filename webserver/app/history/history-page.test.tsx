...
describe('HistoryPage', () => {
    ...
    it('shows loading state initially', () => {
        mockUseAuth.mockReturnValue({
            // Wait, useEffect triggers fetch only if isAuthenticated.
            user: { id: 1 },
            isAuthenticated: true,
            loading: false,
        })
    ...
    it('shows empty state', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByText("You haven't read any articles yet.")
            ).toBeInTheDocument()
        })
    })
    ...
    it('loads more history', async () => {
        const mockArticlesPage1 = [
            { article_id: 1, title: 'History Page 1', readAt: 'd1' },
        ]
        const mockArticlesPage2 = [
            { article_id: 2, title: 'History Page 2', readAt: 'd2' },
        ]
