        ...
        return (
            <div className="container mx-auto px-4 py-12">
                <div className="mb-8 border-b border-gray-200 pb-6">
                    <h1 className="text-3xl md:text-5xl font-serif font-black text-gray-900 mb-4">
                        {feed.name}
                    </h1>
                    <p className="text-xl text-gray-600 font-serif max-w-3xl">
                        {feed.description}
                    </p>
                    <div className="flex flex-wrap gap-2 mt-4">
                        {tags.map((tag) => (
                            <span
                                key={tag.tag_id}
                                className="text-xs font-bold uppercase tracking-wider text-gray-500 bg-gray-100 px-2 py-1 rounded"
                            >
                                #{tag.name}
                            </span>
                        ))}
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                    <p className="text-gray-500">
                        No articles found for these tags yet.
                    </p>
                </div>
            </div>
        )
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-10 border-b-4 border-black pb-6">
                <div className="flex items-start justify-between">
                    <div>
                        <div className="flex items-center gap-4 mb-4">
                            <h1 className="text-4xl md:text-6xl font-serif font-black text-gray-900 tracking-tight">
                                {feed.name}
                            </h1>
                            <FollowButton
                                feedId={feed.id}
                                initialIsFollowing={await getIsFollowingFeed(
                                    feed.id
                                )}
                            />
                        </div>
                        <p className="text-xl text-gray-600 font-serif max-w-3xl leading-relaxed">
                            {feed.description}
                        </p>
                    </div>
                </div>
                <div className="flex flex-wrap gap-2 mt-6">
                    {tags.map((tag) => (
                        <span
                            key={tag.tag_id}
                            className="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-white bg-black px-3 py-1 rounded-full"
                        >
                    ...
                    {publishers.map((pub) => (
                        <span
                            ...
                            className="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-black bg-gray-200 px-3 py-1 rounded-full"
                        >
                            {pub.name}
                        </span>
                    ))}
            ...
            {moreNews.length > 0 && (
                <section className="mt-12 pt-8 border-t border-black">
                    <h5 className="font-bold text-xs uppercase tracking-wider text-gray-900 mb-6">
                        More from {feed.name}
                    </h5>
