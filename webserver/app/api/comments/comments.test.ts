...
describe('Comments API', () => {
    ...
    describe('GET', () => {
        ...
        it('returns count only', async () => {
            const req = new NextRequest('http://api/comments?articleId=1&count=true');
        ...
        it('returns 500 on internal error', async () => {
            const req = new NextRequest('http://api/comments?articleId=1');
            mockPrismaActivity.comment.findMany.mockRejectedValue(new Error('DB Error'));
    ...
    describe('POST', () => {
        it('creates comment successfully', async () => {
            (getSession as any).mockResolvedValue({ user: { id: 1, name: 'Poster' } });

            const req = new NextRequest('http://api/comments', {
                method: 'POST',
                body: JSON.stringify({ articleId: 100, content: 'New Comment' }),
            });

            mockPrismaActivity.comment.create.mockResolvedValue({
                id: 1, content: 'New Comment', createdAt: new Date(), userId: 1, articleId: 100n
            });
            mockPrismaIdentity.user.findUnique.mockResolvedValue({ id: 1, name: 'Poster DB' });
        ...
        it('uses session name if DB user name missing', async () => {
            (getSession as any).mockResolvedValue({ user: { id: 1, name: 'Session Name' } });

            const req = new NextRequest('http://api/comments', { method: 'POST', body: JSON.stringify({ articleId: 100, content: 'C' }) });

            mockPrismaActivity.comment.create.mockResolvedValue({ id: 1 });
            mockPrismaIdentity.user.findUnique.mockResolvedValue({ id: 1, name: null }); // null name in DB
        ...
        it('returns 401 if not authorized', async () => {
            ...
            const req = new NextRequest('http://api/comments', { method: 'POST' });
            const res = await POST(req);
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockPrismaActivity.comment.create.mockRejectedValue(new Error('DB Error'));
