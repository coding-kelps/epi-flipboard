...
describe('Account API Routes', () => {
    ...
    describe('Login Route', () => {
        it('logs in successfully', async () => {
            ...
            mockPrismaIdentity.user.findUnique.mockResolvedValue({
                id: 1, email: 'test@example.com', password: 'hashed-password', name: 'Test User'
            });
        ...
        it('returns 401 if user not found', async () => {
            const req = new NextRequest('http://localhost/api/account/login', {
                method: 'POST',
                body: JSON.stringify({ email: 'unknown@example.com', password: 'password' }),
            });
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockPrismaIdentity.user.findUnique.mockRejectedValue(new Error('DB Error'));
    ...
    describe('Register Route', () => {
        it('registers user successfully', async () => {
            const req = new NextRequest('http://localhost/api/account/register', {
                method: 'POST',
        ...
        it('returns 409 if email exists', async () => {
            const req = new NextRequest('http://localhost/api/account/register', {
                method: 'POST',
                body: JSON.stringify({ email: 'existing@example.com', password: 'password' }),
            });
    ...
    describe('Profile Route', () => {
        describe('GET', () => {
            it('returns user profile', async () => {
                ...
                mockPrismaIdentity.user.findUnique.mockResolvedValue({ id: 1, name: 'User' });
        ...
        describe('PUT', () => {
            it('updates user profile', async () => {
                ...
                mockCookieStore.get.mockReturnValue({ value: 'token' });
                (verifyToken as any).mockReturnValue({ userId: 1 });
                mockPrismaIdentity.user.update.mockResolvedValue({ id: 1, name: 'Updated Name' });
            ...
            it('returns 401 if no token', async () => {
                const req = new NextRequest('http://localhost/api/account/profile', { method: 'PUT' });
                mockCookieStore.get.mockReturnValue(undefined);
                const res = await ProfilePUT(req);
                expect(res.status).toBe(401);
            });
    ...
    describe('Delete Route', () => {
        it('deletes account successfully', async () => {
            ...
            mockPrismaIdentity.user.findUnique.mockResolvedValue({ id: 1, email: 'test@example.com' });
        ...
        it('returns 401 if no token', async () => {
            const req = new NextRequest('http://localhost/api/account/delete', { method: 'DELETE' });
            mockCookieStore.get.mockReturnValue(undefined);
            const res = await DeleteDELETE(req);
            expect(res.status).toBe(401);
        });
        ...
        it('returns 403 if email confirmation does not match', async () => {
            const req = new NextRequest('http://localhost/api/account/delete', {
                method: 'DELETE',
                body: JSON.stringify({ confirmationEmail: 'wrong@example.com' }),
            });
            mockCookieStore.get.mockReturnValue({ value: 'token' });
            (verifyToken as any).mockReturnValue({ userId: 1 });
        ...
        it('returns 500 on internal error', async () => {
            ...
            (verifyToken as any).mockReturnValue({ userId: 1 });
            mockPrismaIdentity.user.findUnique.mockResolvedValue({ id: 1, email: 'test@example.com' });
            mockPrismaActivity.feed.findMany.mockRejectedValue(new Error('DB Error'));
