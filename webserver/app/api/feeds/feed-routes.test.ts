...
describe('Feed API Routes', () => {
    ...
    describe('Create Feed Route', () => {
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockCookieStore.get.mockReturnValue({ value: 'token' })
            ;(verifyToken as any).mockReturnValue({ userId: 1 })
            mockPrismaActivity.feed.create.mockRejectedValue(
                new Error('DB Error')
            )
    ...
    describe('Update Feed Route', () => {
        ...
        it('updates feed if owner', async () => {
            ...
            mockPrismaActivity.feed.findUnique.mockResolvedValue({
                id: 10,
                userId: 1,
            })
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockCookieStore.get.mockReturnValue({ value: 'token' })
            ;(verifyToken as any).mockReturnValue({ userId: 1 })
            mockPrismaActivity.feed.findUnique.mockRejectedValue(
                new Error('DB Error')
            )
