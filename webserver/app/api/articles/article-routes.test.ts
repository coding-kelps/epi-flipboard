...
describe('Article & History API Routes', () => {
    ...
    describe('Mark Article Route', () => {
        it('marks an article', async () => {
            const req = new NextRequest('http://api/articles/mark', {
                method: 'POST',
                body: JSON.stringify({ userId: '1', articleId: '100', action: 'mark' }),
            });
            mockPrismaActivity.markedArticle.create.mockResolvedValue({
                userId: 1, articleId: 100n, createdAt: new Date()
            });
        ...
        it('unmarks an article', async () => {
            const req = new NextRequest('http://api/articles/mark', {
                method: 'POST',
                body: JSON.stringify({ userId: '1', articleId: '100', action: 'unmark' }),
            });
        ...
        it('returns 400 for invalid action', async () => {
            const req = new NextRequest('http://api/articles/mark', {
                method: 'POST',
                body: JSON.stringify({ userId: '1', articleId: '100', action: 'invalid' }),
            });
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockPrismaActivity.markedArticle.create.mockRejectedValue(new Error('DB Error'));
    ...
    describe('Marked Articles Route', () => {
            ...
            (getArticlesByIds as any).mockResolvedValue([
                {
                    ...
                    article_tag: [{ article_id: 100n, tag_id: 1n, tags: { tag_id: 1n, name: 'Tag', created_at: new Date() } }]
                }
            ]);
        ...
        it('returns 500 on internal error', async () => {
            const req = new NextRequest('http://api/articles/marked?userId=1');
            mockPrismaActivity.markedArticle.findMany.mockRejectedValue(new Error('DB Error'));
    ...
    describe('Record History Route', () => {
        ...
        it('records history (update)', async () => {
            ...
            mockPrismaActivity.readingHistory.findFirst.mockResolvedValue({ id: 5 });
        ...
        it('returns 500 on internal error', async () => {
            ...
            mockPrismaActivity.readingHistory.findFirst.mockRejectedValue(new Error('DB Error'));
    ...
    describe('Get History Route', () => {
        ...
        it('returns 500 on internal error', async () => {
            (getSession as any).mockResolvedValue({ user: { id: 1 } });
            const req = new NextRequest('http://api/history');
            mockPrismaActivity.readingHistory.findMany.mockRejectedValue(new Error('DB Error'));
