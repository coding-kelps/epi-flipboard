...
// Mock Components
vi.mock('@/components/ExploreSearch', () => ({ default: () => <div data-testid="explore-search" /> }));
...
describe('Dynamic Pages', () => {
    ...
    describe('Explore Page', () => {
        it('renders feeds from search', async () => {
            const feeds = [{ id: 1, name: 'Tech Feed', description: 'Desc', tagIds: [], createdAt: new Date() }];
            mockSearchFeeds.mockResolvedValue(feeds);

            const jsx = await ExplorePage({ searchParams: Promise.resolve({ q: 'tech' }) });
            render(jsx);
        ...
        it('renders empty state', async () => {
            ...
            const jsx = await ExplorePage({ searchParams: Promise.resolve({ q: 'unknown' }) });
            render(jsx);
    ...
    describe('Search Page', () => {
        it('renders search results', async () => {
            const articles = [{ article_id: 1n, title: 'Result 1', image_url: 'http://img.com/1.jpg' }];
            mockSearchArticles.mockResolvedValue(articles);

            const jsx = await SearchPage({ searchParams: Promise.resolve({ q: 'news' }) });
            render(jsx);

            expect(screen.getByText('Search Results for "news"')).toBeInTheDocument();
    ...
    describe('Saved Page (Client Component)', () => {
        it('fetches and renders saved articles', async () => {
            ...
            mockFetch.mockResolvedValue({
                ok: true,
                json: async () => ({ markedArticles: [{ article_id: 1, title: 'Saved 1' }] })
            });
    ...
    describe('History Page (Client Component)', () => {
        it('fetches and renders history', async () => {
            ...
            mockFetch.mockResolvedValue({
                ok: true,
                json: async () => ({ articles: [{ article_id: 1, title: 'Read 1' }] })
            });
    ...
    describe('Profile Page (Client Component)', () => {
        it('renders user profile and allows updates', async () => {
            ...
            await waitFor(() => {
                expect(mockFetch).toHaveBeenCalledWith('/api/account/profile', expect.objectContaining({
                    method: 'PUT',
