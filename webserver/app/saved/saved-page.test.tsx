...
describe('MarkedArticlesPage', () => {
    beforeEach(() => {
        ...
        mockFetch.mockResolvedValue({
            ok: true,
            json: async () => ({ markedArticles: [], pagination: { hasMore: false } }),
        });
    });
    ...
    it('fetches and renders articles', async () => {
        ...
        mockFetch.mockResolvedValue({
            ok: true,
            json: async () => ({ markedArticles: mockArticles, pagination: { hasMore: false } }),
        });

        render(<MarkedArticlesPage />);

        await waitFor(() => {
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/articles/marked?userId=1'));
        });
    ...
    it('shows empty state', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("You haven't saved any articles yet.")).toBeInTheDocument();
        });
    });
    ...
    it('loads more articles', async () => {
        ...
        mockFetch
            .mockResolvedValueOnce({
                ok: true,
                json: async () => ({ markedArticles: mockArticlesPage1, pagination: { hasMore: true } }),
            })
            .mockResolvedValueOnce({
                ok: true,
                json: async () => ({ markedArticles: mockArticlesPage2, pagination: { hasMore: false } }),
            });
