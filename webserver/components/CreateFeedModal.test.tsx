...
describe('CreateFeedModal', () => {
    ...
    it('renders correctly', () => {
        ...
        expect(
            screen.getByPlaceholderText('e.g. Tech Trends')
        ).toBeInTheDocument()
        expect(
            screen.getByRole('button', { name: 'Create Feed' })
        ).toBeInTheDocument()
    })
    ...
    it('submits form successfully', async () => {
        ...
        await user.type(
            screen.getByPlaceholderText('What is this feed about?'),
            'Cool stuff'
        )
    ...
    it('searches and adds tags', async () => {
        ...
        await waitFor(() => {
            ...
            expect(
                screen.getByText('javascript', { selector: 'button' })
            ).toBeInTheDocument()
        })
        ...
        expect(
            screen.getByText('javascript', { selector: 'span' })
        ).toBeInTheDocument()
    })
    ...
    it('searches and adds publishers', async () => {
        ...
        await waitFor(() => {
            expect(mockFetch).toHaveBeenCalledWith(
                '/api/publishers/search?q=Tech'
            )
            expect(
                screen.getByText('Tech Daily', { selector: 'button' })
            ).toBeInTheDocument()
        })
        ...
        expect(
            screen.getByText('Tech Daily', { selector: 'span' })
        ).toBeInTheDocument()
    })
        ...
        mockFetch.mockImplementation((url) => {
            if (url.includes('/api/tags/search?ids=10')) {
                return Promise.resolve({
                    ok: true,
                    json: async () => [{ tag_id: 10, name: 'FetchedTag' }],
                })
            }
            if (url.includes('/api/publishers/search?ids=20')) {
                ...
                return Promise.resolve({
                    ok: true,
                    json: async () => [
                        { publisher_id: '20', name: 'FetchedPub' },
                    ],
                })
            }
            return Promise.resolve({ ok: true, json: async () => [] })
