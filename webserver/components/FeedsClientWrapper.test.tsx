...
vi.mock('./CreateFeedModal', () => ({
    default: ({ isOpen, onClose, initialData }: any) => isOpen ? (
        <div data-testid="create-feed-modal">
            <button onClick={onClose}>Close</button>
            <span>{initialData ? `Edit ${initialData.name}` : 'Create New'}</span>
        </div>
    ) : null
}));
...
describe('FeedsClientWrapper', () => {
    ...
    it('deletes feed after confirmation', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('/api/feeds/1', { method: 'DELETE' });
        await waitFor(() => {
