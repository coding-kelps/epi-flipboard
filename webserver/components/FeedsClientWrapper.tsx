...
export default function FeedsClientWrapper({ initialFeeds }: { initialFeeds: Feed[] }) {
    ...
    return (
        <>
            <div className="flex items-center justify-between mb-8 border-b border-gray-200 pb-4">
                <h1 className="text-3xl font-serif font-bold text-gray-900">My Feeds</h1>
                <button
            ...
            {initialFeeds.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <p className="text-gray-500 mb-6">Create a custom feed to follow topics you care about.</p>
                </div>
            ) : (
                            ...
                            <article className="h-full p-6 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow flex flex-col">
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        >
                                            {isDeleting === feed.id ? <span className="animate-spin">...</span> : <Trash2 className="w-4 h-4" />}
                                        </button>
                                    </div>
                                </div>
