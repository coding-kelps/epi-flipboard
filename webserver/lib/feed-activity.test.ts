...
import { updateFeedLastVisit, getFollowedFeedsWithMetadata } from './feed-activity';
...
describe('feed-activity', () => {
    ...
    describe('updateFeedLastVisit', () => {
        ...
        it('handles errors gracefully', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Failed to update feed last visit:', expect.any(Error));
    ...
    describe('getFollowedFeedsWithMetadata', () => {
        ...
        it('returns feeds with new article counts', async () => {
            ...
            const feeds = [
                { id: 10, name: 'Tech', description: 'Tech desc', tagIds: [1], publisherIds: [] },
                { id: 20, name: 'Science', description: 'Science desc', tagIds: [2], publisherIds: [5] },
            ];

            mockFollowedFeedFindMany.mockResolvedValue(followedFeeds);
            mockFeedFindMany.mockResolvedValue(feeds);
            (countNewArticles as any).mockResolvedValueOnce(5).mockResolvedValueOnce(3);
        ...
        it('handles missing feed gracefully', async () => {
            ...
            mockFollowedFeedFindMany.mockResolvedValue([{ feedId: 10, lastVisit: new Date() }]);
